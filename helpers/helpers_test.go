...
func TestCalculateTimePeriodWordsFlagCombination(tPtr *testing.T) {

	type arguments struct {
		wordsPresent ctv.TimePeriodWordsPresent
	}
	...
	tests := []struct {
		...
		{
			name: ctv.TEST_NEGATIVE_SUCCESS + ctv.TXT_IS_INVALID,
			arguments: arguments{
				wordsPresent: ctv.TimePeriodWordsPresent{
					Year:    false,
					Quarter: false,
		...
		{
			name: ctv.TEST_POSITIVE_SUCCESS + ctv.TXT_YEAR,
			arguments: arguments{
				wordsPresent: ctv.TimePeriodWordsPresent{
					Year:    true,
					Quarter: false,
		...
		{
			name: ctv.TEST_POSITIVE_SUCCESS + ctv.TXT_QUARTER,
			arguments: arguments{
				wordsPresent: ctv.TimePeriodWordsPresent{
					Year:    false,
					Quarter: true,
		...
		{
			name: ctv.TEST_POSITIVE_SUCCESS + ctv.TXT_MONTH,
			arguments: arguments{
				wordsPresent: ctv.TimePeriodWordsPresent{
					Year:    false,
					Quarter: false,
		...
		{
			name: ctv.TEST_POSITIVE_SUCCESS + ctv.TXT_WEEK,
			arguments: arguments{
				wordsPresent: ctv.TimePeriodWordsPresent{
					Year:    false,
					Quarter: false,
		...
		{
			name: ctv.TEST_POSITIVE_SUCCESS + ctv.TXT_DAY,
			arguments: arguments{
				wordsPresent: ctv.TimePeriodWordsPresent{
					Year:    false,
					Quarter: false,
	...
	for _, ts := range tests {
		tPtr.Run(
			ts.name, func(t *testing.T) {
				if flagCombo = CalculateTimePeriodWordsFlagCombination(ts.arguments.wordsPresent); flagCombo == ctv.VAL_EMPTY || flagCombo == "00000" {
					gotError = true
				} else {
					gotError = false
				}
