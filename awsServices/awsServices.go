...
func LoadAWSCustomerSettings(
	...
	default:
		if environment == ctv.VAL_EMPTY {
			errorInfo = errs.NewErrorInfo(pi.ErrEnvironmentInvalid, fmt.Sprintf("%v%v", ctv.LBL_ENVIRONMENT, ctv.FN_ENVIRONMENT))
			return
		}
		errorInfo = errs.NewErrorInfo(pi.ErrEnvironmentInvalid, fmt.Sprintf("%v%v", ctv.LBL_ENVIRONMENT, environment))
	}
	...
	if awsSettings.BaseConfig, errorInfo.Error = awsCfg.LoadDefaultConfig(awsCTXToDo, awsCfg.WithRegion(awsSettings.STYHCognitoIdentityInfo.region)); errorInfo.Error != nil {
		errorInfo = errs.NewErrorInfo(pi.ErrServiceFailedAWS, "Failed to create an AWS Session.")
		return
	}
...
func Login(
	...
	if loginType == ctv.VAL_EMPTY {
		errorInfo = errs.NewErrorInfo(pi.ErrRequiredArgumentMissing, fmt.Sprintf("%v%v", ctv.LBL_LOGIN_TYPE, loginType))
		return
	}
	if username == ctv.VAL_EMPTY {
		errorInfo = errs.NewErrorInfo(pi.ErrRequiredArgumentMissing, fmt.Sprintf("%v%v", ctv.LBL_USERNAME, username))
		return
	}
	if password == nil {
		errorInfo = errs.NewErrorInfo(pi.ErrRequiredArgumentMissing, fmt.Sprintf("%v%v", ctv.LBL_PASSWORD, ctv.TXT_PROTECTED))
		return
	}
	if STYHCognitoIdentityInfo.UserPoolId == ctv.VAL_EMPTY {
		errorInfo = errs.NewErrorInfo(pi.ErrRequiredArgumentMissing, fmt.Sprintf("%v%v", ctv.LBL_AWS_USERPOOL_ID, STYHCognitoIdentityInfo.UserPoolId))
		return
	}
	...
	if tCognitoClientPtr = awsCIP.NewFromConfig(baseConfig); tCognitoClientPtr == nil {
		errorInfo = errs.NewErrorInfo(pi.ErrServiceFailedAWS, fmt.Sprintf("%v%v", ctv.LBL_SERVICE, ctv.TXT_AWS_COGNITO))
		return
	}
...
func ParseAWSJWT(
	...
	if len(awsSettings.keyInfo.keySet.Keys) == ctv.VAL_ZERO {
		errorInfo = errs.NewErrorInfo(pi.ErrRequiredArgumentMissing, ctv.TXT_KEY_SET_MISSING)
		return
	}
	if tokenType == ctv.VAL_EMPTY {
		errorInfo = errs.NewErrorInfo(pi.ErrRequiredArgumentMissing, fmt.Sprintf("%v%v", ctv.LBL_TOKEN_TYPE, ctv.FN_TOKEN_TYPE))
		return
	}
	if token == ctv.VAL_EMPTY {
		errorInfo = errs.NewErrorInfo(pi.ErrRequiredArgumentMissing, fmt.Sprintf("%v%v", ctv.LBL_TOKEN, ctv.FN_TOKEN))
		return
	}
...
func PullCognitoUserInfo(
	...
	if username == ctv.VAL_EMPTY {
		errorInfo = errs.NewErrorInfo(pi.ErrRequiredArgumentMissing, fmt.Sprintf("%v%v", ctv.LBL_USERNAME, ctv.FN_USERNAME))
		return
	}
	if userPoolId == ctv.VAL_EMPTY {
		errorInfo = errs.NewErrorInfo(pi.ErrRequiredArgumentMissing, fmt.Sprintf("%v%v", ctv.LBL_AWS_USERPOOL_ID, "userPoolId"))
		return
	}
