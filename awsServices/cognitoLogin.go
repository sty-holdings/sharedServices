...
func hexHash(hexStr string) string {
	...
	if tHexStrByte, errorInfo.Error = hex.DecodeString(hexStr); errorInfo.Error != nil {
		pi.PrintErrorInfo(pi.NewErrorInfo(pi.ErrDecodeStringFailed, fmt.Sprintf("%v%v", ctv.LBL_HEX_STRING, hexStr)))
	}
...
func hexToBig(hexStr string) *big.Int {
	...
	if i, ok = big.NewInt(0).SetString(hexStr, 16); ok == false {
		pi.PrintErrorInfo(pi.NewErrorInfo(pi.ErrSetStringFailed, fmt.Sprintf("%v%v", ctv.LBL_HEX_STRING, hexStr)))
	}
