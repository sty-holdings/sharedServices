...
func NewGCPService(aiConfigFilename string) (gcpServicePtr *GCPService, errorInfo errs.ErrorInfo) {
	...
	if errorInfo = hlps.CheckValueNotEmpty(ctv.LBL_GRP_SERVER, aiConfigFilename, errs.ErrEmptyRequiredParameter, ctv.FN_SERVICE_CONFIG_FILENAME); errorInfo.Error != nil {
		return
	}
...
func loadGCPConfig(gcpConfigFilename string) (gcpConfig GCPConfig, errorInfo errs.ErrorInfo) {
	...
	if errorInfo = hlps.CheckValueNotEmpty(ctv.LBL_GRP_SERVER, gcpConfigFilename, errs.ErrEmptyRequiredParameter, ctv.FN_CONFIG_FILENAME); errorInfo.Error != nil {
		return
	}

	if tConfigData, errorInfo.Error = os.ReadFile(hlps.PrependWorkingDirectory(gcpConfigFilename)); errorInfo.Error != nil {
		errorInfo = errs.NewErrorInfo(errorInfo.Error, errs.BuildLabelValue(ctv.LBL_GRP_SERVER, ctv.LBL_CONFIG_EXTENSION_FILENAME, gcpConfigFilename))
		return
	}
...
func validateGCPConfig(gcpConfig GCPConfig) (errorInfo errs.ErrorInfo) {
	...
	if errorInfo = hlps.CheckValueNotEmpty(ctv.LBL_GRP_SERVER, gcpConfig.GCPCredentialFilename, errs.ErrEmptyRequiredParameter, ctv.FN_GCP_CREDENTIAL_FILENAME); errorInfo.Error != nil {
		return
	}
	...
	if errorInfo = hlps.CheckValueNotEmpty(ctv.LBL_GRP_SERVER, gcpConfig.GCPLocation, errs.ErrEmptyRequiredParameter, ctv.FN_GCP_LOCATION); errorInfo.Error != nil {
		return
	}
	errorInfo = hlps.CheckValueNotEmpty(ctv.LBL_GRP_SERVER, gcpConfig.GCPProjectID, errs.ErrEmptyRequiredParameter, ctv.FN_GCP_PROJECT_ID)
